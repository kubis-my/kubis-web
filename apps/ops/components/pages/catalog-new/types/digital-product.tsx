...
interface CreateDigitalProductResponse {
    createDigitalProductForOps: OpsProduct;
}

interface CreateDigitalProductVariables {
    companyPublicId: string;
    input: DigitalProductInput;
}

const CREATE_DIGITAL_PRODUCT: TypedDocumentNode<
    CreateDigitalProductResponse,
    CreateDigitalProductVariables
> = gql`
    mutation CreateDigitalProduct($companyPublicId: String!, $input: DigitalProductInput!) {
        createDigitalProductForOps(companyPublicId: $companyPublicId, input: $input) {
            publicId
            name
        }
    }
...
export function DigitalProductForm({ onClose, onDirtyChange }: { onClose: () => void; onDirtyChange?: (dirty: boolean) => void }) {
    const [form, setForm] = useState<FormState>(DEFAULT_FORM);
    const [formValidation, setFormValidation] = useState<Record<string, string[]>>({});
    const { activeCompany } = useCompany();
    const client = useApolloClient();
    const [createDigitalProduct, { loading }] = useMutation(CREATE_DIGITAL_PRODUCT);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            const { data, error } = await createDigitalProduct({
                variables: {
    ...
    return (
        <form id="digital-product-form" onSubmit={handleSubmit} className="flex w-full flex-col">
            <section className="flex flex-col gap-4 py-4">
                <div>
                    <p className="text-sm font-medium">Basic Information</p>
                    <p className="text-muted-foreground text-xs">
                        Define how this digital item is represented in catalog and sales records.
                    </p>
                </div>
            ...
            <section className="flex flex-col gap-4 py-4">
                <div>
                    ...
                    <p className="text-muted-foreground text-xs">
                        Set the default selling price for this digital product.
                    </p>
                </div>

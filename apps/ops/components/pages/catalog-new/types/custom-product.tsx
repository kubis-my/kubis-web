...
interface CreateCustomProductResponse {
    createCustomProductForOps: OpsProduct;
}

interface CreateCustomProductVariables {
    companyPublicId: string;
    input: CustomProductInput;
}

const CREATE_CUSTOM_PRODUCT: TypedDocumentNode<
    CreateCustomProductResponse,
    CreateCustomProductVariables
> = gql`
    mutation CreateCustomProduct($companyPublicId: String!, $input: CustomProductInput!) {
        createCustomProductForOps(companyPublicId: $companyPublicId, input: $input) {
            publicId
            name
        }
    }
...
export function CustomProductForm({ onClose, onDirtyChange }: { onClose: () => void; onDirtyChange?: (dirty: boolean) => void }) {
    const [form, setForm] = useState<FormState>(DEFAULT_FORM);
    const [formValidation, setFormValidation] = useState<Record<string, string[]>>({});
    const { activeCompany } = useCompany();
    const client = useApolloClient();
    const [createCustomProduct, { loading }] = useMutation(CREATE_CUSTOM_PRODUCT);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            const { data, error } = await createCustomProduct({
                variables: {

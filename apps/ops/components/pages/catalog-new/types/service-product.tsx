...
interface CreateServiceProductResponse {
    createServiceProductForOps: OpsProduct;
}

interface CreateServiceProductVariables {
    companyPublicId: string;
    input: ServiceProductInput;
}

const CREATE_SERVICE_PRODUCT: TypedDocumentNode<
    CreateServiceProductResponse,
    CreateServiceProductVariables
> = gql`
    mutation CreateServiceProduct($companyPublicId: String!, $input: ServiceProductInput!) {
        createServiceProductForOps(companyPublicId: $companyPublicId, input: $input) {
            publicId
            name
        }
    }
...
export function ServiceProductForm({
    ...
    const [createServiceProduct, { loading }] = useMutation(CREATE_SERVICE_PRODUCT);
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        try {
            const { data, error } = await createServiceProduct({
                variables: {
    ...
    return (
        <form id="service-product-form" onSubmit={handleSubmit} className="flex w-full flex-col">
            <section className="flex flex-col gap-4 py-4">
                <div>
                    <p className="text-sm font-medium">Basic Information</p>
                    <p className="text-muted-foreground text-xs">
                        Define how this service appears in your product list and ordering flows.
                    </p>
                </div>
            ...
            <section className="flex flex-col gap-4 py-4">
                <div>
                    ...
                    <p className="text-muted-foreground text-xs">
                        Set the base service fee before discounts or adjustments.
                    </p>
                </div>

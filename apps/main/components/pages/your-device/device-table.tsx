...
export default function DeviceTable() {
    ...
    const [paginatedCredentialDevice, setPaginatedCredentialDevice] = useState<
        Omit<PaginatedCredentialDevice, 'overview'>
    ...
    const goToNextPage = useCallback(() => {
        if (
            paginatedCredentialDevice.pageInfo.hasNextPage &&
            paginatedCredentialDevice.pageInfo.endCursor !== null
        ) {
    ...
    useEffect(() => {
        setPaginatedCredentialDevice(
            data?.getCredentialDevices ??
                ctx.paginatedCredentialDevice ??
                createInitialPaginatedData(),
        );
    }, [ctx.paginatedCredentialDevice, data?.getCredentialDevices]);

    if (ctx.isFetchingCredentialDevice)

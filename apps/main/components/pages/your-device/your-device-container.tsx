...
const GET_CREDENTIAL_DEVICES: TypedDocumentNode<
...
export default function YourDeviceContainer({ children }: Readonly<{ children: React.ReactNode }>) {
    ...
    const [paginatedCredentialDevice, setPaginatedCredentialDevice] = useState<
        PaginatedCredentialDevice | undefined
    >(undefined);
    ...
    const signOutAllOtherDevices = useCallback(async () => {
        try {
            ...
            if (hasGraphQLError(error)) {
                toast.error('Failed to revoke access from other devices', {
                    position: 'top-center',
                });
                return;
            }
    ...
    const otherDevicesCount = useMemo(() => {
        return (
            paginatedCredentialDevice?.data.filter(
    ...
    useEffect(() => {
        updateBreadcrumbList([{ name: 'Your Devices' }]);
        updateHeaderAction(
            <DeviceHeaderAction
                otherDevicesCount={otherDevicesCount}
                onSignOutAll={signOutAllOtherDevices}
            />,
        );
    ...
    return <YourDeviceContext.Provider value={contextValue}>{children}</YourDeviceContext.Provider>;
}

...
export default function AddUserDialog() {
    ...
    const [searchCredentials, { loading: isSearching, data: searchData }] = useLazyQuery(SEARCH_CREDENTIALS_FOR_BRANCH, {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (hasGraphQLError(error)) {
                ...
                if (gqlError) {
                    const err = gqlError.extensions?.originalError as Record<string, unknown> | undefined;
                        ...
                        {selectedUser ? (
                            <div className="flex items-center gap-3 p-3 rounded-lg border border-border bg-muted/30">
                                <Avatar className="size-10 rounded-full">
                                    <AvatarImage src={selectedUser.profilePicture ?? undefined} alt={selectedUser.displayName} />
                            ...
                            <>
                                ...
                                <div className="flex flex-col w-full max-h-[250px] overflow-y-auto rounded-lg border border-border">
                                    {searchQuery.trim() === "" ? (
                                        ...
                                        searchResults.map((user) => (
                                            ...
                                            >
                                                <Avatar className="size-10 rounded-full">
                                                    <AvatarImage src={user.profilePicture ?? undefined} alt={getDisplayName(user)} />
                                                    <AvatarFallback className="rounded-full bg-muted">
                                                ...
                                                <IconCheck className="size-5 text-transparent group-hover:text-muted-foreground shrink-0" />
                                            </button>
                                        ))
                                    )}
                                </div>
